...
class TalismanismSkillWindow(QMainWindow):
    def __init__(self, session, db_path, max_skill, max_tier, skill_points, spend_skill_points_window):
        super().__init__()
        ...
        self.setWindowTitle("Talismanism Skills")
    ...
    def get_skills_by_tier(self, tier):
        with self.db_path as conn:
            cur = conn.cursor()
            cur.execute("SELECT * FROM master_skills WHERE tier = ? AND attribute = 'Intelligence' AND type = 'talisman'", (tier,))
            return cur.fetchall()
        ...
        for skill_id, (points_label, add_button, subtract_button) in self.skill_buttons.items():
            ...
            if skill:
                ...
                add_button.setDisabled(
                    ...
                    or (parent_skill_id is not None and self.session.skills_purchased.get(parent_skill_id, 0) < 1)
                )
