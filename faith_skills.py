...
class FaithSkillWindow(QMainWindow):
    def __init__(self, session, db_path, max_skill, max_tier, skill_points, spend_skill_points_window):
        ...
        self.setWindowTitle("Faith Skills")
    ...
    def get_skills_by_tier(self, tier):
        with self.db_path as conn:
            cur = conn.cursor()
            cur.execute("SELECT * FROM master_skills WHERE tier = ? AND attribute = 'Wisdom' AND type = 'faith'", (tier,))
            return cur.fetchall()
