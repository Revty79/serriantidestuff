...
class StrengthSkillWindow(QMainWindow):
    def __init__(self, session, db_path, max_skill, next_tier, max_tier, skill_points, allowed_skills, spend_skill_points_window):
        ...
        self.setWindowTitle("Strength Skills")
    ...
    def get_skills_by_tier(self, tier):
        with self.db_path as conn:
            cur = conn.cursor()
            cur.execute("SELECT * FROM master_skills WHERE tier = ? AND attribute = 'Strength' AND type = 'normal'", (tier,))
            return cur.fetchall()
